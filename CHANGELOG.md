...
## 1.3.1 2026/02/05
- カスタムオブジェクトにおいて，拡大率を変えるとアンカーポイントと描画がズレるバグを修正．
- 折れ線矢印の描画を調整．
...
## 1.4.0 2026/03/18
- スレッドプール方式にし，スレッド数を指定するパラメータを削除．

## 1.4.1 2026/03/23
- 完全透明のピクセルを含むオブジェクトに画素平均法リサイズを適用した場合に，ゼロ除算でクラッシュするバグを修正．

## 1.4.2 2026/04/02
- 丸めを四捨五入から `std::nearbyint` 及び明示的な最近接偶数丸めに変更．
- 累積的なコーディングの修正．
...
## 2.0.0 2026/02/05
- AviUtl ExEdit2 用を追加．可能なパラメータの設定法が異なるため，無印用とは一部インターフェースが異なる．

## 2.0.1 2026/03/02
- KSAインターレース解除のスクリプトが壊れていたバグを修正．
- 開発環境において，マルチスレッドのオーバーヘッドが非常に大きいため，デフォルトスレッド数を1にしたうえで，スレッド数が1のとき，`std::thread`を使わず，直接処理関数をよぶようにした．
- その他，内部の軽微な修正．

## 2.1.0 2026/03/04
- スレッドプール方式にし，スレッド数を指定するパラメータを削除．マルチスレッド処理が実用的になった．
